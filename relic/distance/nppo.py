...
def df_groupby_check_new(d1, d2, df_dict, g_inferred, debug=False, strict_schema=False, lattice_check=False,
    ...
    column_diff = ppo.set_jaccard_similarity(set(df1.columns), set(df2.columns))
...
def score_join_schema(df1, df2, df3, df_dict, debug=False, replay=True):
    ...
    for join_dest in combo_set:
        ...
        jaccard = ppo.set_jaccard_similarity(columns_dict[join_dest], column_union) * len(
            ...
            for col in coldict[max_col_score]:
                for val in valdict[max_val_score]:
                    try:
                        ...
                        similarities.append(ppo.compute_jaccard_DF(dst, replaydf))
...
def replay_merge(src1, src2, dst, key, threshold=0.95):
    ...
    try:
        ...
        inner_score = ppo.compute_jaccard_DF(dst, inner_result, containment=False)
        if inner_score < threshold:
            left_result = src1.merge(src2, on=key, how='left')
            left_score = ppo.compute_jaccard_DF(dst, left_result, containment=False)
