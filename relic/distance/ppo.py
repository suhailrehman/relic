...
def compute_all_ppo(df1, df2, ppo='all', pk_col_name=None, reindex=False,
    ...
    try:
        ...
        logging.warning("Reindex error. Ignoring.", e)
        pass
    ...
    if len(common_cols) == 0:
        logger.debug('No common cols', comparison_cols, common_cols, df3.head())
        return _return_zero(ppo)
...
def compute_col_jaccard_df(df1, df2):
    ...
    for col in common_cols:
        try:
            ...
            logging.debug('col:', col, 'jaccard:', sim)
        except Exception as e:
            print(col)
            print(set_jaccard_similarity(set(df1[col].values), set(df2[col].values)))
            raise e

    logging.debug('num/denom', np.sum(common_cols_jaccard), len(set(df1).union(set(df2))))
